...
def findMaxPeakIndex(ind, array):
    count = len(array)
    while ind < (count - 1) and array[ind] != 0 and array[ind + 1] <= array[ind]:
        ind += 1
    return ind - 1
...
def detectFeatures(df, inputFile, paramFile):
    ...
    scanWindow = gap + 1
    checkTol = float(params["mz_tolerance_metabolite_feature"])
        ...
        for j in range(pCount):
            ...
            for k in range(i - 1, minInd - 1, -1):
                ...
                if q["m/z array"].size == 0:
                    ...
                    match, ind = getClosest(q, cm, matchTol)
                if match == 1:
                    break
                nTry += 1
                if nTry > scanWindow:
        ...
        for j in range(pCount):
            ...
            for k in range(i - 1, minInd - 1, -1):
                ...
                if q["m/z array"].size == 0:
                    ...
                    matchIndicator, ind = getClosest(q, cm, matchTol)
