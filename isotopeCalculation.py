...
# Create a dictionary with isotopic peak intensity and mass for the mono elements (with cutoff)
...
# Module to convert an amino acid sequence to a chemical element dictionary
...
def iso_distri_(iso_mass_inten_dict_trimmed, chemical_com, Charge, isotope_cutoff, mass_tolerance, select_weighted_mass, select_strong_intensity_peaks ):
    ...
    if any(chemical_com[k] > default_elementDict_size[k] for k in set(chemical_com).intersection(default_elementDict_size)): #if any(k > 200 for k in list(chemical_com.values())):
        for element, count in list(chemical_com.items())[:1]:
            if count > default_elementDict_size[element]:
                pep_iso_distr_df = iso_distri_largeNum(element, count, iso_mass_inten_dict_trimmed)
            else: #if elemnt count is not grater than deafaults size       
                pep_iso_distr_df = pd.DataFrame((iso_mass_inten_dict_trimmed[element]['Intensity'][count]), columns = ['isotope_inten'])
                pep_iso_distr_df['isotope_mass'] =  (iso_mass_inten_dict_trimmed[element]['Mass'][count])
            
        for element, count in list(chemical_com.items())[1:]:
            if count > default_elementDict_size[element]:
                pep_iso_distr_df_ = iso_distri_largeNum(element, count,iso_mass_inten_dict_trimmed)
                ...
                pep_iso_distr_df = iso_distri_combine_eleme(pep_iso_distr_df, iso_mass_inten_dict_trimmed[element]['Intensity'][count], iso_mass_inten_dict_trimmed[element]['Mass'][count], mass_tolerance, select_weighted_mass, select_strong_intensity_peaks)
...
def getIsotopicDistributions(paramFile, inputFile):
    ...
    with open(preComputedIsotopes, 'rb') as f:
        iso_mass_inten_dict_trimmed = pickle.load(f)
    ...
    if 'Tracer_1' in params:
        if params['Tracer_1'] == '13C':
            elemInfo_dict.update({"x": {12: 1-float(params['Tracer_1_purity']), 13.00335483521: float(params['Tracer_1_purity'])}})
            # print("\nTracer element '13C' is represented with letter 'x'\n ")
        elif params['Tracer_1'] == '15N':
            elemInfo_dict.update({"y": {14.00307400446: 1-float(params['Tracer_1_purity']), 15.0001088989: float(params['Tracer_1_purity'])}})
    ...
    if 'Tracer_2' in params:
        if params['Tracer_2'] == '13C':
            elemInfo_dict.update({"x": {12: 1-float(params['Tracer_2_purity']), 13.00335483521: float(params['Tracer_2_purity'])}})
            # print("\nTracer element '13C' is represented with letter 'x'\n ")
        elif params['Tracer_2'] == '15N':
            elemInfo_dict.update({"y": {14.00307400446: 1-float(params['Tracer_2_purity']), 15.0001088989: float(params['Tracer_2_purity'])}})
            # print("\nTracer element '15N' is represented with letter 'y'\n ")
        else:
            print("\n Information for parameter \"Tracer_2\" is not properly defined\n ")

    iso_mass_inten_dict_trimmed = isotope_distribution_indElement(elemInfo_dict, iso_mass_inten_dict_trimmed, inten_threshold_trim = 1e-10)
    ...
    for i in range(0, len(inputDf)):
        ...
        for j in range(0, chemical_com["C"]+1):
            ...
            iso_distr_temp = pd.DataFrame(0, index=np.arange(0, chemical_com["C"]+1), columns= ['isotope_mass', 'isotope_inten'])
            ...
            iso_distr_temp.loc[j-len(iso_distr[iso_distr.isotope_mass < iso_distr.isotope_mass.iloc[0]]):j-1,:] = iso_distr[iso_distr.isotope_mass < iso_distr.isotope_mass.iloc[0]].values
            iso_distr_all = iso_distr_all.append({'isotopologues': 'M'+str(j),'isotope_m/z': ';'.join([str(f) for f in iso_distr_temp.isotope_mass.values]), 'isotope_intensity': ';'.join([str(f) for f in iso_distr_temp.isotope_inten.values])}, ignore_index=True)
