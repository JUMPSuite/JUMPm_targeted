...
##################################################################################################################################
import re, numpy as np, pandas as pd, pickle, sys, os
        ...
        for line in file:
            ...
            if "feature_files" in parameters and line.endswith("feature"):
                ...
                if key == "feature_files":
                    ...
                    parameters[key] = val
    return parameters
 #### function read text file to dataframe by skipping the top rows untill the desired starting string is observed
...
print("\nGoing to update the elementary dictionary with the tracer element with their defined Natural_abundance/purity\n ")
...
print("\nElement isotope dictionary is updated with tracer elements (eg. C13)\n")
...
input_file = read_df_skip_topRows(params['input_file'],"Index", sep="\t")
#input_file.set_index('Index',inplace=True)
print('\n Input- file is imported \n')
print("\nCalculating the isotopic distribution for the metabolite  topologues\n")
...
for i in range(0,len(input_file)):
    ...
    for j in range(0, chemical_com["C"]+1):
        ...
        iso_distr_all = iso_distr_all.append({'iso_topologus': 'M'+str(j),'isotope_M/Z': ';'.join([str(f) for f in iso_distr_temp.isotope_mass.values]), 'isotope_intensity': ';'.join([str(f) for f in iso_distr_temp.isotope_inten.values])}, ignore_index=True)
        inputData = inputData.append(input_file.loc[[i]], ignore_index=True) #, 
...
outfile = open(params['output_file']+".txt","w") 
iso_distr_all.to_csv(outfile, sep = "\t", line_terminator='\n', mode='a',na_rep='NA',index=False)#
outfile.close()
